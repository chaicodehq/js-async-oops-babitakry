...
export class HaveliSecurity {
  ...
  addResident(name, role, passcode) {
    ...
    const roleArray = ["malik", "naukar", "mehmaan"]
    ...
    const nameFound = this.#residents.find((r) => r.name === name);
  ...
  verifyAccess(name) {
    const allowed = this.#residents.some(r => r.name === name);
  ...
  isResident(name) {
    const exist = this.#residents.some((r) => r.name === name);
